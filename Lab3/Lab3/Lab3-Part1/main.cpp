...
//function to display when a key is pressed and when the key is released
...
//displays the keyboard modifiers that are active, e.g. Numlock or Capslock
...
//function to display the key user pressed
...
int main(int argc, char *argv[])
{
	...
	while (!done)
	{
		...
		while (SDL_PollEvent(&event))
		{
			switch (event.type)
			{
				//when keyboard key is pressed down, call these functions
			case SDL_KEYDOWN:
				DisplayState(&event.key);
				DisplayModifiers(&event.key);
				DisplayKey(&event.key);
				break;
				//when keyboard key is no longer being pressed, call these functions
			case SDL_KEYUP:
				DisplayState(&event.key);
				DisplayModifiers(&event.key);
				DisplayKey(&event.key);
